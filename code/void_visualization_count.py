...
def Visualization(net,model_name, xdata, ydata, num_test):
    ...
    for i in range(num_test):
        ...
        x = Variable(x).cuda()
        output_stats,output_map,output_source  = net(x)
        ...
        x=x[0][0]
        output_stats=output_stats.detach().cpu().numpy()

        print('void count ground truth: ',y,'    prediction: ',output_stats)

        print('#-------------------------------------------------------------------------')
        result[0,i]=y[0]
        result[1,i]=output_stats[0]
...
model_names = ['densenet_7','squeezenet_7','resnet_7_count1124']
...
xdata=np.load('void_project/data/xTest_count.npy')
ydata=np.load('void_project/data/yTest_count.npy')
result=Visualization(net, mod, xdata, ydata, num_test)
del net
np.save('void_project/data/count_graph.npy',result)
